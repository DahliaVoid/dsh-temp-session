 ...
 *      Workspace 记录（保持"不选工作区"的会话始终 Ungrouped）；同时注销桌面
 *      客户端「未分组」功能在 `$DSH_HOME/ungrouped` 留下的伪项目记录；删除既不
 *      在存活 Agent 也不在持久化列表中的、超过 7 天的临时目录。
...
async function cleanup(ctx, tempRoot) {
	try {
		...
		//
		// 桌面客户端（dsh-tauri 的 dsh-tauri-ui）自带的「未分组」新建会话会把
		// cwd 落到 `$DSH_HOME/ungrouped`（0.15.9 起），同样会被物化成一条伪项目
		// 分组。本插件接管"无工作区会话"语义后该目录不再产生新会话，这里顺带
		// 注销它的历史残留记录（会话与日志同样不动）。
		const registry = ctx.workspaceRegistry;
		if (registry !== undefined && typeof registry.list === "function") {
			const desktopUngroupedRoot = join(resolveDshHome(), "ungrouped");
			for (const workspace of registry.list()) {
				if (isUnderRoot(tempRoot, workspace.path) || isUnderRoot(desktopUngroupedRoot, workspace.path)) {
